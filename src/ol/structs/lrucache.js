...
ol.structs.LRUCache.prototype.assertValid = function() {
  if (this.count_ === 0) {
    ...
    goog.asserts.assert(this.oldest_ === null,
        'oldest must be null (count = 0)');
    goog.asserts.assert(this.newest_ === null,
    ...
    goog.asserts.assert(this.oldest_.older === null,
        'no entry is older than oldest');
    goog.asserts.assert(this.newest_,
        'we have a newest entry');
    goog.asserts.assert(this.newest_.newer === null,
        'no entry is newer than newest');
    var i, entry;
    var older = null;
    i = 0;
    for (entry = this.oldest_; entry !== null; entry = entry.newer) {
    ...
    var newer = null;
    i = 0;
    for (entry = this.newest_; entry !== null; entry = entry.older) {
...
ol.structs.LRUCache.prototype.peekLast = function() {
  goog.asserts.assert(this.oldest_ !== null, 'oldest must not be null');
...
ol.structs.LRUCache.prototype.pop = function() {
  goog.asserts.assert(this.oldest_ !== null, 'oldest must not be null');
  goog.asserts.assert(this.newest_ !== null, 'newest must not be null');
  var entry = this.oldest_;
