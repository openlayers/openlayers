...
import { StringType, NumberType, ColorType, BooleanType, newParsingContext } from '../../expr/expression.js';
import {
...
class WebGLTextLayerRenderer extends WebGLLayerRenderer {
    ...
    constructor(layer, options) {
        ...
        this.atlas_ = new GlyphAtlas(options.fontFamily || 'sans-serif', options.fontWeight || 'normal');
    ...
    rebuildBuffers_(frameState) {
        ...
        let textEvaluator, fontSizeEvaluator, fillColorEvaluator, strokeColorEvaluator, strokeWidthEvaluator, visibleEvaluator;
        const evalContext = newEvaluationContext();

        if (textExpression) {
            textEvaluator = buildExpression(textExpression, StringType, newParsingContext());
        }
        if (fontSizeExpression) {
            fontSizeEvaluator = buildExpression(fontSizeExpression, NumberType, newParsingContext());
        }
        if (fillColorExpression) {
            fillColorEvaluator = buildExpression(fillColorExpression, ColorType, newParsingContext());
        }
        if (strokeColorExpression) {
            strokeColorEvaluator = buildExpression(strokeColorExpression, ColorType, newParsingContext());
        }
        if (strokeWidthExpression) {
            strokeWidthEvaluator = buildExpression(strokeWidthExpression, NumberType, newParsingContext());
        }
        if (visibleExpression) {
            visibleEvaluator = buildExpression(visibleExpression, BooleanType, newParsingContext());
        }
        ...
        features.forEach(feature => {
            ...
            if (textEvaluator) {
                ...
                text = feature.get('text') || feature.get('name') || feature.get('label') || '';
            }
            if (!text) return;
            text = String(text);
            ...
            if (this.bidiInstance_) {
                try {
                    ...
                    console.warn('[TextLayer] BiDi failed:', e);
                }
            }
            ...
            const backgroundOutlineColor = feature.get('backgroundOutlineColor') || backgroundColor;
            const backgroundOutlineWidth = feature.get('backgroundOutlineWidth') !== undefined ? feature.get('backgroundOutlineWidth') : 0.0;
            const hasBackground = backgroundColor[3] > 0 || (backgroundOutlineWidth > 0 && backgroundOutlineColor[3] > 0);
            ...
            const userSpacing = feature.get('spacing') !== undefined ? feature.get('spacing') : 0;
            ...
            if (hasBackground && whiteGlyph) {
                ...
                const bgR = backgroundColor[0], bgG = backgroundColor[1], bgB = backgroundColor[2], bgA = backgroundColor[3];
                const bgOR = backgroundOutlineColor[0], bgOG = backgroundOutlineColor[1], bgOB = backgroundOutlineColor[2], bgOA = backgroundOutlineColor[3];
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, this.atlas_.getCanvas());
