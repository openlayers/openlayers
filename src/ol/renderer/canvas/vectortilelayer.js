...
ol.renderer.canvas.VectorTileLayer = function(layer) {
  ...
  this.zDirection = layer.getRenderMode() ==
 ...
 * @inheritDoc
 */
ol.renderer.canvas.VectorTileLayer.prototype.prepareFrame = function(
  frameState,
  layerState
) {
  ...
  return ol.renderer.canvas.TileLayer.prototype.prepareFrame.apply(
    this,
    arguments
  );
};
...
ol.renderer.canvas.VectorTileLayer.prototype.createReplayGroup_ = function(
  ...
  for (var t = 0, tt = tile.tileKeys.length; t < tt; ++t) {
    ...
    var sourceTileResolution = sourceTileGrid.getResolution(
      sourceTile.tileCoord[0]
    );
    ...
    if (tileProjection.getUnits() == ol.proj.Units.TILE_PIXELS) {
      ...
      var transform = ol.transform.compose(
        ...
        0,
        0,
        1 / sourceTileResolution * tilePixelRatio,
        -1 / sourceTileResolution * tilePixelRatio,
        0,
        -sourceTileExtent[0],
        -sourceTileExtent[3]
      ...
      extent = ol.transform
        .apply(transform, [sharedExtent[0], sharedExtent[3]])
        .concat(
          ol.transform.apply(transform, [sharedExtent[2], sharedExtent[1]])
        );
    } else {
...
ol.renderer.canvas.VectorTileLayer.prototype.forEachFeatureAtCoordinate = function(
  ...
  var source /** @type {ol.source.VectorTile} */ = layer.getSource();
  var tileGrid = source.getTileGridForProjection(
  ...
  for (i = 0, ii = renderedTiles.length; i < ii; ++i) {
    ...
    bufferedExtent = ol.extent.buffer(
      tileExtent,
    ...
    for (var t = 0, tt = tile.tileKeys.length; t < tt; ++t) {
      ...
      if (sourceTile.getProjection().getUnits() === ol.proj.Units.TILE_PIXELS) {
        var sourceTileCoord = sourceTile.tileCoord;
        var sourceTileExtent = sourceTileGrid.getTileCoordExtent(
          sourceTileCoord,
          this.tmpExtent
        );
        ...
        tileResolution =
          sourceTileGrid.getResolution(sourceTileCoord[0]) / tilePixelRatio;
        tileSpaceCoordinate = [
      ...
      found =
        found ||
        replayGroup.forEachFeatureAtCoordinate(
          tileSpaceCoordinate,
          resolution,
          rotation,
          hitTolerance,
 ...
 * @private
 */
ol.renderer.canvas.VectorTileLayer.prototype.getReplayTransform_ = function(
  tile,
  frameState
) {
 ...
 * @private
 */
ol.renderer.canvas.VectorTileLayer.prototype.handleStyleImageChange_ = function(
  event
) {
 ...
 * @inheritDoc
 */
ol.renderer.canvas.VectorTileLayer.prototype.postCompose = function(
  context,
  frameState,
  layerState
) {
  ...
  if (replays) {
    ...
    for (var i = tiles.length - 1; i >= 0; --i) {
      ...
      var tileCoord = tile.tileCoord;
      var worldOffset =
        tileGrid.getTileCoordExtent(tileCoord)[0] -
      ...
      for (var t = 0, tt = tile.tileKeys.length; t < tt; ++t) {
        ...
        ol.transform.translate(
          transform,
          worldOffset * tilePixelRatio / sourceResolution,
          0
        );
 ...
 */
ol.renderer.canvas.VectorTileLayer.prototype.renderFeature = function(
  feature,
  squaredTolerance,
  styles,
  replayGroup
...
ol.renderer.canvas.VectorTileLayer.prototype.renderTileImage_ = function(
  ...
  var revision = layer.getRevision();
  var replays =
    ol.renderer.canvas.VectorTileLayer.IMAGE_REPLAYS[layer.getRenderMode()];
  if (replays && replayState.renderedTileRevision !== revision) {
    ...
    var size = source.getTilePixelSize(
      z,
      pixelRatio,
      frameState.viewState.projection
    );
    ...
    for (var i = 0, ii = tile.tileKeys.length; i < ii; ++i) {
      ...
      if (sourceTile.getProjection().getUnits() == ol.proj.Units.TILE_PIXELS) {
        ...
        var renderPixelRatio =
          pixelRatio / tilePixelRatio * sourceResolution / resolution;
        ol.transform.scale(transform, renderPixelRatio, renderPixelRatio);
        var offsetX =
          (sourceTileExtent[0] - tileExtent[0]) /
          sourceResolution *
          tilePixelRatio;
        var offsetY =
          (tileExtent[3] - sourceTileExtent[3]) /
          sourceResolution *
          tilePixelRatio;
        ol.transform.translate(
          transform,
