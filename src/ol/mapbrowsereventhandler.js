...
ol.MapBrowserEventHandler = function(map, moveTolerance) {
   ...
   * @private
   */
  this.pointerdownListenerKey_ = ol.events.listen(
   ...
   * @private
   */
  this.relayedListenerKey_ = ol.events.listen(
    this.pointerEventHandler_,
...
ol.MapBrowserEventHandler.prototype.emulateClick_ = function(pointerEvent) {
  ...
  if (this.clickTimeoutId_ !== 0) {
    ...
    newEvent = new ol.MapBrowserPointerEvent(
      ol.MapBrowserEventType.DBLCLICK,
      this.map_,
      pointerEvent
    );
    this.dispatchEvent(newEvent);
    ...
    this.clickTimeoutId_ = setTimeout(
      function() {
        this.clickTimeoutId_ = 0;
        var newEvent = new ol.MapBrowserPointerEvent(
          ol.MapBrowserEventType.SINGLECLICK,
          this.map_,
          pointerEvent
        );
        this.dispatchEvent(newEvent);
 ...
 * @private
 */
ol.MapBrowserEventHandler.prototype.updateActivePointers_ = function(
  pointerEvent
) {
  var event = pointerEvent;

  if (
    event.type == ol.MapBrowserEventType.POINTERUP ||
 ...
 * @private
 */
ol.MapBrowserEventHandler.prototype.isMouseActionButton_ = function(
  pointerEvent
) {
...
ol.MapBrowserEventHandler.prototype.handlePointerDown_ = function(
  ...
  if (this.dragListenerKeys_.length === 0) {
     ...
     */
    this.documentPointerEventHandler_ = new ol.pointer.PointerEventHandler(
      document
    );
 ...
 * @private
 */
ol.MapBrowserEventHandler.prototype.handlePointerMove_ = function(
  pointerEvent
) {
  ...
  if (this.isMoving_(pointerEvent)) {
    this.dragging_ = true;
    var newEvent = new ol.MapBrowserPointerEvent(
      ol.MapBrowserEventType.POINTERDRAG,
      this.map_,
      pointerEvent,
      this.dragging_
    );
