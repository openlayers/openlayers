...
if (ol.ENABLE_WEBGL) {
  ...
  ol.render.webgl.PolygonReplay.prototype.drawCoordinates_ = function(
    ...
    var maxX = this.processFlatCoordinates_(
      flatCoordinates,
      stride,
      outerRing,
      rtree,
    ...
    if (holeFlatCoordinates.length) {
      ...
      for (i = 0, ii = holeFlatCoordinates.length; i < ii; ++i) {
        ...
        holeList.maxX = this.processFlatCoordinates_(
          holeFlatCoordinates[i],
          stride,
          holeList.list,
          holeList.rtree,
          false
  ...
  ol.render.webgl.PolygonReplay.prototype.processFlatCoordinates_ = function(
    ...
    if (clockwise === isClockwise) {
      ...
      start = this.createPoint_(
        flatCoordinates[end],
        flatCoordinates[end + 1],
        n++
      );
      p0 = start;
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.classifyPoints_ = function(
    list,
    rtree,
    ccw
  ) {
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.bridgeHole_ = function(
    hole,
    holeMaxX,
    list,
    listMaxX,
    ...
    for (i = 0, ii = intersectingSegments.length; i < ii; ++i) {
      ...
      var intersection = this.calculateIntersection_(
        p1,
        p2,
        currSeg.p0,
        currSeg.p1,
    ...
    if (minDist > 0) {
      ...
      if (pointsInTriangle.length) {
        ...
        for (i = 0, ii = pointsInTriangle.length; i < ii; ++i) {
          ...
          if (
            currTheta < theta ||
            (currTheta === theta && currPoint.x < bestPoint.x)
          ) {
            theta = currTheta;
            bestPoint = currPoint;
          }
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.clipEars_ = function(
    list,
    rtree,
    simple,
    ccw
    ...
    do {
      ...
      if (p1.reflex === false) {
        ...
        var diagonalIsInside = ccw
          ? this.diagonalIsInside_(s3.p1, p2, p1, p0, s0.p0)
          : this.diagonalIsInside_(s0.p0, p0, p1, p2, s3.p1);
        if (
          (simple ||
            this.getIntersections_({p0: p0, p1: p2}, rtree).length === 0) &&
          diagonalIsInside &&
          this.getPointsInTriangle_(p0, p1, p2, rtree, true).length === 0
        ) {
  ...
  */
    ...
    do {
      ...
      if (intersection) {
        ...
        if (opt_touch) {
          ...
          rtree.update(
            [
              Math.min(s0.p0.x, s0.p1.x),
              Math.min(s0.p0.y, s0.p1.y),
              Math.max(s0.p0.x, s0.p1.x),
              Math.max(s0.p0.y, s0.p1.y)
            ],
            s0
          );
          rtree.update(
            [
              Math.min(s1.p0.x, s1.p1.x),
              Math.min(s1.p0.y, s1.p1.y),
              Math.max(s1.p0.x, s1.p1.x),
              Math.max(s1.p0.y, s1.p1.y)
            ],
            s1
          );
        }
  ...
  ol.render.webgl.PolygonReplay.prototype.isClockwise_ = function(list) {
    ...
    return ol.geom.flat.orient.linearRingIsClockwise(
      flatCoordinates,
      0,
      length,
      2
   ...
   * @param {ol.structs.RBush} rtree R-Tree of the polygon.
   */
  ol.render.webgl.PolygonReplay.prototype.splitPolygon_ = function(
    list,
    rtree
  ) {
    var start = list.firstItem();
    ...
    do {
      ...
      if (intersections.length) {
        ...
        rtree.update(
          [
            Math.min(s0.p0.x, p.x),
            Math.min(s0.p0.y, p.y),
            Math.max(s0.p0.x, p.x),
            Math.max(s0.p0.y, p.y)
          ],
          s0
        );
        ...
        rtree.update(
          [
            Math.min(s1.p1.x, p.x),
            Math.min(s1.p1.y, p.y),
            Math.max(s1.p1.x, p.x),
            Math.max(s1.p1.y, p.y)
          ],
          s1
        );
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.insertItem_ = function(
    p0,
    p1,
    list,
    opt_rtree
  ...
  /**
    ...
    * @param {ol.structs.RBush} rtree R-Tree of the polygon.
    */
  ol.render.webgl.PolygonReplay.prototype.removeItem_ = function(
    s0,
    s1,
    list,
    rtree
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.getPointsInTriangle_ = function(
    p0,
    p1,
    p2,
    rtree,
    ...
    var i, ii, j, p;
    var result = [];
    var segmentsInExtent = rtree.getInExtent([
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.getIntersections_ = function(
    segment,
    rtree,
    opt_touch
  ) {
    ...
    for (i = 0, ii = segmentsInExtent.length; i < ii; ++i) {
      ...
      if (
        segment !== currSeg &&
        (opt_touch || currSeg.p0 !== p1 || currSeg.p1 !== p0) &&
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.calculateIntersection_ = function(
    p0,
    p1,
    p2,
    p3,
    ...
    if (denom !== 0) {
      var ua =
        ((p3.x - p2.x) * (p0.y - p2.y) - (p3.y - p2.y) * (p0.x - p2.x)) / denom;
      var ub =
        ((p1.x - p0.x) * (p0.y - p2.y) - (p1.y - p0.y) * (p0.x - p2.x)) / denom;
      if (
        (!opt_touch &&
          ua > ol.render.webgl.EPSILON &&
          ua < 1 - ol.render.webgl.EPSILON &&
          ub > ol.render.webgl.EPSILON &&
          ub < 1 - ol.render.webgl.EPSILON) ||
        (opt_touch && ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1)
      ) {
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.diagonalIsInside_ = function(
    p0,
    p1,
    p2,
    p3,
    ...
    var p3IsLeftOf =
      (p0.x - p1.x) * (p3.y - p1.y) > (p0.y - p1.y) * (p3.x - p1.x);
    var p3IsRightOf =
      (p2.x - p1.x) * (p3.y - p1.y) < (p2.y - p1.y) * (p3.x - p1.x);
    var p1InCone = p3.reflex
      ? p1IsRightOf || p1IsLeftOf
      : p1IsRightOf && p1IsLeftOf;
    var p3InCone = p1.reflex
      ? p3IsRightOf || p3IsLeftOf
      : p3IsRightOf && p3IsLeftOf;
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.drawMultiPolygon = function(
    ...
    for (i = 0, ii = endss.length; i < ii; ++i) {
      var ends = endss[i];
      if (ends.length > 0) {
        var outerRing = ol.geom.flat.transform.translate(
          flatCoordinates,
          start,
          ends[0],
          stride,
        ...
        if (outerRing.length) {
          ...
          for (j = 1, jj = ends.length; j < jj; ++j) {
            if (ends[j] !== ends[j - 1]) {
              holeFlatCoords = ol.geom.flat.transform.translate(
                flatCoordinates,
                ends[j - 1],
                ends[j],
                stride,
          ...
          this.lineStringReplay.drawPolygonCoordinates(
            outerRing,
            holes,
            stride
          );
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.drawPolygon = function(
    polygonGeometry,
    feature
  ) {
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.getDeleteResourcesFunction = function(
    context
  ) {
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.setUpProgram = function(
    gl,
    context,
    size,
    pixelRatio
    ...
    if (!this.defaultLocations_) {
      ...
      locations = new ol.render.webgl.polygonreplay.defaultshader.Locations(
        gl,
        program
      );
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.shutDownProgram = function(
    gl,
    locations
  ) {
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.drawReplay = function(
    gl,
    context,
    skippedFeaturesHash,
    hitDetection
    ...
    var tmpDepthMask /** @type {boolean} */ = gl.getParameter(
      gl.DEPTH_WRITEMASK
    );
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.drawHitDetectionReplayOneByOne = function(
    gl,
    context,
    skippedFeaturesHash,
    featureCallback,
   ...
   */
  ol.render.webgl.PolygonReplay.prototype.drawReplaySkipping_ = function(
    gl,
    context,
    skippedFeaturesHash
  ) {
   ...
   * @inheritDoc
   */
  ol.render.webgl.PolygonReplay.prototype.setFillStrokeStyle = function(
    fillStyle,
    strokeStyle
  ) {
