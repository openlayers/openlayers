...
import {equals, getCenter} from '../extent.js';
...
class RasterSource extends ImageSource {
  ...
  constructor(options) {
     ...
     * @type {Array<boolean>}
     */
    this.sampleArrayData_ = options.sources.map(
      (sourceOrLayer) =>
        sourceOrLayer instanceof Source && readsArrayData(sourceOrLayer),
    );
  ...
  updateFrameState_(extent, resolution, projection) {
    ...
    // align the extent to the pixel grid of the resolution
    const minX = Math.floor(extent[0] / resolution) * resolution;
    const minY = Math.floor(extent[1] / resolution) * resolution;
    const maxX = Math.ceil(extent[2] / resolution) * resolution;
    const maxY = Math.ceil(extent[3] / resolution) * resolution;
    frameState.size[0] = Math.round((maxX - minX) / resolution);
    frameState.size[1] = Math.round((maxY - minY) / resolution);
    frameState.extent = [minX, minY, maxX, maxY];
    frameState.time = Date.now();

    const viewState = frameState.viewState;
    viewState.center = getCenter(frameState.extent);
  ...
  getImage(extent, resolution, pixelRatio, projection) {
    ...
    // snap to the input resolutions, and beyond the coarsest one continue in
    // powers of two, so operations only run at discrete resolutions and the
    // composed image stays within a factor of two of the viewport size.
    const resolutions = this.getResolutions(projection);
    if (resolutions) {
      const coarsest = resolutions[0];
      resolution =
        resolution > coarsest
          ? coarsest * Math.pow(2, Math.round(Math.log2(resolution / coarsest)))
          : resolutions[linearFindNearest(resolutions, resolution, 0)];
    }
    ...
    if (
      this.tileQueue_.getCount() > 0 ||
      this.tileQueue_.getTilesLoading() > 0
    ) {
   ...
   * @private
   */
  getInput_(layerIndex, frameState) {
    const layer = this.layers_[layerIndex];
    if (this.sampleArrayData_[layerIndex]) {
      const renderer =
        /** @type {import("../renderer/canvas/TileLayer.js").default} */ (
          /** @type {*} */ (layer.getRenderer())
        );
      if (!renderer.prepareFrame(frameState)) {
        return null;
      }
      return renderer.readData(frameState);
    }
    frameState.layerIndex = layerIndex;
    frameState.renderTargets = {};
    return getImageData(layer, frameState);
  }
  ...
  getData(coordinate) {
    ...
    for (let i = 0, ii = this.layers_.length; i < ii; ++i) {
      ...
      if (this.sampleArrayData_[i]) {
        // array data tile inputs are gathered as native band values through the
        // layer renderer over a one-pixel extent around the coordinate
        const renderer =
          /** @type {import("../renderer/canvas/TileLayer.js").default} */ (
            /** @type {*} */ (layer.getRenderer())
          );
        const resolution = frameState.viewState.resolution;
        const lookupFrameState = /** @type {import("../Map.js").FrameState} */ (
        ...
        const input = renderer.readData(lookupFrameState);
        if (!input) {
          return null;
        }
  ...
  processSources_() {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      const input = this.getInput_(i, frameState);
      if (!input) {
        return;
      }
      inputs[i] = input;
...
/**
 * The source's data should be sampled directly as typed arrays
 * rather than rendered to an image and read back.
