 ...
 *
 * @param {EventTarget} el Event target element (the map Canvas element)
...
export function getCanvasScale(el) {
  ...
  const scaleX =
    (el.getBoundingClientRect().width * DEVICE_PIXEL_RATIO) / el.offsetWidth;
  const scaleY =
    (el.getBoundingClientRect().height * DEVICE_PIXEL_RATIO) / el.offsetHeight;
