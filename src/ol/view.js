...
ol.View.prototype.applyOptions_ = function(options) {
  ...
  properties[ol.ViewProperty.ROTATION] = options.rotation !== undefined
    ? options.rotation
    : 0;
...
ol.View.prototype.getResolution = function() {
  return /** @type {number|undefined} */ this.get(ol.ViewProperty.RESOLUTION);
};
...
ol.View.prototype.getZoomForResolution = function(resolution) {
  ...
  if (resolution >= this.minResolution_ && resolution <= this.maxResolution_) {
    ...
    if (this.resolutions_) {
      var nearest = ol.array.linearFindNearest(
        this.resolutions_,
        resolution,
        1
      );
...
ol.View.prototype.fit = function(geometryOrExtent, opt_options) {
  ...
  if (!(geometryOrExtent instanceof ol.geom.SimpleGeometry)) {
    ol.asserts.assert(Array.isArray(geometryOrExtent), 24); // Invalid extent or geometry provided as `geometry`
    ol.asserts.assert(!ol.extent.isEmpty(geometryOrExtent), 25); // Cannot fit empty extent provided as `geometry`
...
ol.View.createResolutionConstraint_ = function(options) {
  ...
  if (options.resolutions !== undefined) {
    ...
    var defaultMaxResolution =
      ...
      Math.pow(defaultZoomFactor, ol.DEFAULT_MIN_ZOOM);

    var defaultMinResolution =
      defaultMaxResolution /
      Math.pow(defaultZoomFactor, defaultMaxZoom - ol.DEFAULT_MIN_ZOOM);
