...
describe('ol.VectorTile', () => {

  test('loader reprojects GeoJSON features', done => {
    ...
    listen(tile, 'change', function(e) {
      expect(tile.getFeatures()[0].getGeometry().getFlatCoordinates()).toEqual([-9724792.346778862, 4164041.638405114]);
      done();
    });
    loader.call(tile, [], 1, getProjection('EPSG:3857'));
  });

  test('loader reprojects MVT features', done => {
    ...
    const loader = tile.loader_;
    listen(tile, 'change', function(e) {
      expect(tile.getFeatures()[1246].getGeometry().getFlatCoordinates()).toEqual([1827804.0218549764, 6144812.116688028]);
      done();
    });
