...
describe('ol.format.WMTSCapabilities', function() {
  ...
  describe('when parsing ign.xml', function() {
    ...
    it('can read Capability.Contents.Layer', function() {
      ...
      var layer = capabilities.Contents.Layer[0];
      expect(layer.Abstract).to.be
        .eql('Photographies aériennes');
      expect(layer.Identifier).to.be.eql('ORTHOIMAGERY.ORTHOPHOTOS');
      expect(layer.Title).to.be.eql('Photographies aériennes');

      expect(layer.Format).to.be.an('array');
      expect(layer.Format).to.have.length(1);
      expect(layer.Format[0]).to.be.eql('image/jpeg');

      expect(layer.Style).to.be.an('array');
      expect(layer.Style).to.have.length(1);
      expect(layer.Style[0].Identifier).to.be.eql('normal');
      expect(layer.Style[0].isDefault).to.be(true);
      expect(layer.Style[0].Title).to.be.eql('Données Brutes');
      expect(layer.Style[0].LegendURL[0].href).to.be
        .eql('http://www.geoportail.gouv.fr/depot/LEGEND.jpg');
      expect(layer.Style[0].LegendURL[0].format).to.be.eql('image/jpeg');
      ...
      expect(wgs84Bbox[0]).to.be.eql(-180);
      expect(wgs84Bbox[2]).to.be.eql(180);
      expect(wgs84Bbox[1]).to.be.eql(-86);
      expect(wgs84Bbox[3]).to.be.eql(84);
