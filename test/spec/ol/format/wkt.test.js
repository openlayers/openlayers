...
describe('ol.format.WKT', () => {
  ...
  describe('#readGeometry()', () => {
    ...
    test('transforms with dataProjection and featureProjection', () => {
      const wkt = 'POINT(1 2)';
      const geom = format.readGeometry(wkt, {
        dataProjection: 'EPSG:4326',
        featureProjection: 'EPSG:3857'
      });
      expect(geom.getCoordinates()).toEqual(transform([1, 2], 'EPSG:4326', 'EPSG:3857'));
    });
  ...
  describe('#writeFeature()', () => {
    ...
    test('transforms with dataProjection and featureProjection', () => {
      ...
      expect(gotFeature).toBeInstanceOf(Feature);
  ...
  describe('#readFeatures()', () => {
    ...
    test('transforms with dataProjection and featureProjection', () => {
      ...
      expect(point1.getType()).toEqual('Point');
      expect(point2.getType()).toEqual('Point');
      expect(point1.getCoordinates()).toEqual(transform([1, 2], 'EPSG:4326', 'EPSG:3857'));
      expect(point2.getCoordinates()).toEqual(transform([4, 5], 'EPSG:4326', 'EPSG:3857'));
    });
  });
  ...
  describe('#writeFeatures()', () => {
    ...
    test('transforms with dataProjection and featureProjection', () => {
      ...
      expect(gotFeatures).toHaveLength(2);
  ...
  test('Point read / written correctly', () => {
    let wkt = 'POINT(30 10)';
    let geom = format.readGeometry(wkt);
    expect(geom.getCoordinates()).toEqual([30, 10]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('Point Z read / written correctly', () => {
    let wkt = 'POINT Z(30 10 5)';
    let geom = format.readGeometry(wkt);
    expect(geom.getCoordinates()).toEqual([30, 10, 5]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
    wkt = 'POINT Z (30 10 5)';
    geom = format.readGeometry(wkt);
  ...
  test('Point M read / written correctly', () => {
  ...
  test('Point ZM read / written correctly', () => {
    let wkt = 'POINT ZM(30 10 5 0.1)';
    let geom = format.readGeometry(wkt);
    expect(geom.getCoordinates()).toEqual([30, 10, 5, 0.1]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('MultiPoint read / written correctly', () => {
    ...
    expect(points[0].getCoordinates()).toEqual([10, 40]);
    expect(points[1].getCoordinates()).toEqual([40, 30]);
    expect(points[2].getCoordinates()).toEqual([20, 20]);
    expect(points[3].getCoordinates()).toEqual([30, 10]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // this has whitespace
  ...
  test('MultiPoint Z read / written correctly', () => {
    // there are two forms to test
    let wkt = 'MULTIPOINT Z((10 40 1),(40 30 2),(20 20 3),(30 10 4))';
    let geom = format.readGeometry(wkt);
    let points = geom.getPoints();
    expect(points.length).toEqual(4);
    expect(points[0].getCoordinates()).toEqual([10, 40, 1]);
    expect(points[1].getCoordinates()).toEqual([40, 30, 2]);
    expect(points[2].getCoordinates()).toEqual([20, 20, 3]);
    expect(points[3].getCoordinates()).toEqual([30, 10, 4]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('MultiPoint M read / written correctly', () => {
    // there are two forms to test
  ...
  test('MultiPoint ZM read / written correctly', () => {
    // there are two forms to test
    let wkt = 'MULTIPOINT ZM((10 40 1 0.1),(40 30 2 0.1),(20 20 3 0.1),(30 10 4 0.1))';
    let geom = format.readGeometry(wkt);
    let points = geom.getPoints();
    expect(points.length).toEqual(4);
    expect(points[0].getCoordinates()).toEqual([10, 40, 1, 0.1]);
    expect(points[1].getCoordinates()).toEqual([40, 30, 2, 0.1]);
    expect(points[2].getCoordinates()).toEqual([20, 20, 3, 0.1]);
    expect(points[3].getCoordinates()).toEqual([30, 10, 4, 0.1]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // this has whitespace and no standardized parentheses
  ...
  test('LineString read / written correctly', () => {
    let wkt = 'LINESTRING(30 10,10 30,40 40)';
    let geom = format.readGeometry(wkt);
    expect(geom.getType()).toEqual('LineString');
    expect(geom.getCoordinates()).toEqual([[30, 10], [10, 30], [40, 40]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('LineString Z read / written correctly', () => {
    let wkt = 'LINESTRING Z(30 10 1,10 30 2,40 40 3)';
    let geom = format.readGeometry(wkt);
    expect(geom.getType()).toEqual('LineString');
    expect(geom.getCoordinates()).toEqual([[30, 10, 1], [10, 30, 2], [40, 40, 3]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('LineString M read / written correctly', () => {
  ...
  test('LineString ZM read / written correctly', () => {
    let wkt = 'LINESTRING ZM(30 10 1 0.1,10 30 2 0.1,40 40 3 0.1)';
    let geom = format.readGeometry(wkt);
    expect(geom.getType()).toEqual('LineString');
    expect(geom.getCoordinates()).toEqual([[30, 10, 1, 0.1], [10, 30, 2, 0.1], [40, 40, 3, 0.1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('MultiLineString read / written correctly', () => {
    let wkt = 'MULTILINESTRING((10 10,20 20,10 40),' +
        '(40 40,30 30,40 20,30 10))';
    let geom = format.readGeometry(wkt);
    expect(geom.getType()).toEqual('MultiLineString');
    let linestrings = geom.getLineStrings();
    expect(linestrings.length).toEqual(2);
    expect(linestrings[0].getType()).toEqual('LineString');
    expect(linestrings[0].getCoordinates()).toEqual([[10, 10], [20, 20], [10, 40]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('MultiLineString Z read / written correctly', () => {
    ...
    expect(linestrings.length).toEqual(2);
    expect(linestrings[0].getType()).toEqual('LineString');
    expect(linestrings[0].getCoordinates()).toEqual([[10, 10, 1], [20, 20, 2], [10, 40, 3]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('MultiLineString M read / written correctly', () => {
  ...
  test('MultiLineString ZM read / written correctly', () => {
    ...
    expect(linestrings.length).toEqual(2);
    expect(linestrings[0].getType()).toEqual('LineString');
    expect(linestrings[0].getCoordinates()).toEqual([[10, 10, 1, 0.1], [20, 20, 2, 0.1], [10, 40, 3, 0.1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('Polygon read / written correctly', () => {
    let wkt = 'POLYGON((30 10,10 20,20 40,40 40,30 10))';
    let geom = format.readGeometry(wkt);
    expect(geom.getType()).toEqual('Polygon');
    let rings = geom.getLinearRings();
    expect(rings.length).toEqual(1);
    expect(rings[0].getType()).toEqual('LinearRing');
    expect(rings[0].getCoordinates()).toEqual([[30, 10], [10, 20], [20, 40], [40, 40], [30, 10]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    ...
    expect(rings[0].getType()).toEqual('LinearRing');
    expect(rings[1].getType()).toEqual('LinearRing');
    expect(rings[0].getCoordinates()).toEqual([[35, 10], [10, 20], [15, 40], [45, 45], [35, 10]]);
    expect(rings[1].getCoordinates()).toEqual([[20, 30], [30, 20], [35, 35], [20, 30]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('Polygon Z read / written correctly', () => {
    ...
    expect(rings.length).toEqual(1);
    expect(rings[0].getType()).toEqual('LinearRing');
    expect(rings[0].getCoordinates()).toEqual([[30, 10, 1], [10, 20, 2], [20, 40, 3], [40, 40, 4], [30, 10, 1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    ...
    expect(rings[0].getType()).toEqual('LinearRing');
    expect(rings[1].getType()).toEqual('LinearRing');
    expect(rings[0].getCoordinates()).toEqual([[35, 10, 1], [10, 20, 2], [15, 40, 3], [45, 45, 4], [35, 10, 1]]);
    expect(rings[1].getCoordinates()).toEqual([[20, 30, 1], [30, 20, 2], [35, 35, 3], [20, 30, 1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('Polygon M read / written correctly', () => {
  ...
  test('Polygon ZM read / written correctly', () => {
    ...
    expect(rings.length).toEqual(1);
    expect(rings[0].getType()).toEqual('LinearRing');
    expect(rings[0].getCoordinates()).toEqual(
    ...
    expect(rings[1].getCoordinates()).toEqual([[20, 30, 1, 0.1], [30, 20, 2, 0.1], [35, 35, 3, 0.1], [20, 30, 1, 0.1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('MultiPolygon read / written correctly', () => {
    // note that WKT doesn't care about winding order, we do
    let wkt = 'MULTIPOLYGON(((40 40,45 30,20 45,40 40)),' +
        '((20 35,45 20,30 5,10 10,10 30,20 35),(30 20,20 25,20 15,30 20)))';
    let geom = format.readGeometry(wkt);
    expect(geom.getType()).toEqual('MultiPolygon');
    let polygons = geom.getPolygons();
    expect(polygons.length).toEqual(2);
    expect(polygons[0].getType()).toEqual('Polygon');
    expect(polygons[1].getType()).toEqual('Polygon');
    expect(polygons[0].getLinearRings().length).toEqual(1);
    expect(polygons[1].getLinearRings().length).toEqual(2);
    expect(polygons[0].getLinearRings()[0].getCoordinates()).toEqual([[40, 40], [45, 30], [20, 45], [40, 40]]);
    expect(polygons[1].getLinearRings()[0].getCoordinates()).toEqual([[20, 35], [45, 20], [30, 5], [10, 10], [10, 30], [20, 35]]);
    expect(polygons[1].getLinearRings()[1].getCoordinates()).toEqual([[30, 20], [20, 25], [20, 15], [30, 20]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('MultiPolygon Z read / written correctly', () => {
    // note that WKT doesn't care about winding order, we do
    ...
    expect(polygons[0].getLinearRings().length).toEqual(1);
    expect(polygons[1].getLinearRings().length).toEqual(2);
    expect(polygons[0].getLinearRings()[0].getCoordinates()).toEqual([[40, 40, 1], [45, 30, 2], [20, 45, 3], [40, 40, 1]]);
    expect(polygons[1].getLinearRings()[0].getCoordinates()).toEqual(
      [[20, 35, 1], [45, 20, 2], [30, 5, 3], [10, 10, 4], [10, 30, 5], [20, 35, 1]]
    );
    expect(polygons[1].getLinearRings()[1].getCoordinates()).toEqual([[30, 20, 1], [20, 25, 2], [20, 15, 3], [30, 20, 1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('MultiPolygon M read / written correctly', () => {
    // note that WKT doesn't care about winding order, we do
  ...
  test('MultiPolygon ZM read / written correctly', () => {
    // note that WKT doesn't care about winding order, we do
    ...
    expect(polygons[0].getLinearRings().length).toEqual(1);
    expect(polygons[1].getLinearRings().length).toEqual(2);
    expect(polygons[0].getLinearRings()[0].getCoordinates()).toEqual([[40, 40, 1, 0.1], [45, 30, 2, 0.1], [20, 45, 3, 0.1], [40, 40, 1, 0.1]]);
    expect(polygons[1].getLinearRings()[0].getCoordinates()).toEqual(
      [[20, 35, 1, 0.1], [45, 20, 2, 0.1], [30, 5, 3, 0.1], [10, 10, 4, 0.1], [10, 30, 5, 0.1], [20, 35, 1, 0.1]]
    );
    expect(polygons[1].getLinearRings()[1].getCoordinates()).toEqual([[30, 20, 1, 0.1], [20, 25, 2, 0.1], [20, 15, 3, 0.1], [30, 20, 1, 0.1]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  ...
  test('Empty geometries read / written correctly', () => {
    const wkt = 'POINT EMPTY';
    const geom = format.readGeometry(wkt);
    const coordinates = geom.getCoordinates();
    expect(coordinates.length).toBe(2);
    expect(isNaN(coordinates[0])).toBe(true);
    expect(isNaN(coordinates[1])).toBe(true);
    const wkts = [
  ...
  test('Invalid geometries detected correctly', () => {
    expect(function() {
  ...
  test('GeometryCollection read / written correctly', () => {
    let wkt = 'GEOMETRYCOLLECTION(POINT(4 6),LINESTRING(4 6,7 10))';
    let geom = format.readGeometry(wkt);
    let geoms = geom.getGeometries();
    expect(geoms.length).toEqual(2);
    expect(geom.getType()).toEqual('GeometryCollection');
    expect(geoms[0].getType()).toEqual('Point');
    expect(geoms[1].getType()).toEqual('LineString');
    expect(geoms[0].getCoordinates()).toEqual([4, 6]);
    expect(geoms[1].getCoordinates()).toEqual([[4, 6], [7, 10]]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
    // test whitespace when reading
  ...
  test('Empty GeometryCollection read / written correctly', () => {
    const wkt = 'GEOMETRYCOLLECTION EMPTY';
    const geom = format.readGeometry(wkt);
    expect(geom.getGeometries()).toEqual([]);
    expect(format.writeGeometry(geom)).toEqual(wkt);
  });

  test('GeometryCollection split / merged correctly', () => {
    ...
    expect(geoms[0].getCoordinates()).toEqual([4, 6]);
    expect(geoms[1].getCoordinates()).toEqual([[4, 6], [7, 10]]);
    expect(format.writeFeatures(features)).toEqual(wkt);
  });

  test('Point feature read / written correctly', () => {
    const wkt = 'POINT(30 10)';
    const feature = format.readFeature(wkt);
    const geom = feature.getGeometry();
    expect(geom.getCoordinates()).toEqual([30, 10]);
    expect(format.writeFeature(feature)).toEqual(wkt);
  });

  test('Features read / written correctly', () => {
    ...
    expect(point1.getType()).toEqual('Point');
    expect(point2.getType()).toEqual('Point');
    expect(point1.getCoordinates()).toEqual([1, 2]);
    expect(point2.getCoordinates()).toEqual([3, 4]);
    expect(format.writeFeatures(features)).toEqual(wkt);
  });

  describe('scientific notation supported', () => {

    test('handles scientific notation correctly', () => {
      const wkt = 'POINT(3e1 1e1)';
      const geom = format.readGeometry(wkt);
      expect(geom.getCoordinates()).toEqual([30, 10]);
      expect(format.writeGeometry(geom)).toEqual('POINT(30 10)');
    });

    test('works with with negative exponent', () => {
      const wkt = 'POINT(3e-1 1e-1)';
      const geom = format.readGeometry(wkt);
      expect(geom.getCoordinates()).toEqual([0.3, 0.1]);
      expect(format.writeGeometry(geom)).toEqual('POINT(0.3 0.1)');
    });

    test('works with with explicitly positive exponent', () => {
      const wkt = 'POINT(3e+1 1e+1)';
      const geom = format.readGeometry(wkt);
      expect(geom.getCoordinates()).toEqual([30, 10]);
      expect(format.writeGeometry(geom)).toEqual('POINT(30 10)');
    ...
    test('handles very small numbers in scientific notation', () => {
      // very small numbers keep the scientific notation, both when reading and
      // writing
      const wkt = 'POINT(3e-9 1e-9)';
      const geom = format.readGeometry(wkt);
      expect(geom.getCoordinates()).toEqual([3e-9, 1e-9]);
      expect(format.writeGeometry(geom)).toEqual('POINT(3e-9 1e-9)');
    });

    test('handles very big numbers in scientific notation', () => {
      // very big numbers keep the scientific notation, both when reading and
      // writing
      const wkt = 'POINT(3e25 1e25)';
      const geom = format.readGeometry(wkt);
      expect(geom.getCoordinates()).toEqual([3e25, 1e25]);
      expect(format.writeGeometry(geom)).toEqual('POINT(3e+25 1e+25)');
    });

    test('works case insensitively (e / E)', () => {
      const wkt = 'POINT(3E1 1E1)';
      const geom = format.readGeometry(wkt);
      expect(geom.getCoordinates()).toEqual([30, 10]);
      expect(format.writeGeometry(geom)).toEqual('POINT(30 10)');
    ...
    test('detects invalid scientific notation', () => {
      expect(function() {
        // note the double 'e'
