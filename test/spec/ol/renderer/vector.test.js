  ...
  describe('#renderFeature', function() {
    ...
    describe('call multiple times', function() {
      it('does not set multiple listeners', function() {
        ...
        listeners = ol.events.getListeners(iconStyle.iconImage_, 'change');
    ...
    describe('call renderFeature with a loading icon', function() {
      it('does not render the point', function() {
        ...
        var imageReplay = replayGroup.getReplay(style.getZIndex(), 'Image');
        var setImageStyleSpy = sinon.spy(imageReplay, 'setImageStyle');
        var drawPointSpy = sinon
          .stub(imageReplay, 'drawPoint')
          .callsFake(ol.nullFunction);
        ol.renderer.vector.renderFeature(
          replayGroup,
          feature,
          style,
      ...
      it('does render the linestring', function() {
        ...
        var setFillStrokeStyleSpy = sinon.spy(
          lineStringReplay,
          'setFillStrokeStyle'
        );
        var drawLineStringSpy = sinon
      ...
      it('does render the multilinestring', function() {
        ...
        var drawMultiLineStringSpy = sinon
      ...
      it('does render the polygon', function() {
        feature.setGeometry(
          new ol.geom.Polygon([[[0, 0], [1, 1], [1, 0], [0, 0]]])
        );
        var polygonReplay = replayGroup.getReplay(style.getZIndex(), 'Polygon');
        var setFillStrokeStyleSpy = sinon.spy(
          polygonReplay,
          'setFillStrokeStyle'
        );
        var drawPolygonSpy = sinon
      ...
      it('does render the multipolygon', function() {
        feature.setGeometry(
          new ol.geom.MultiPolygon([[[[0, 0], [1, 1], [1, 0], [0, 0]]]])
        );
        var polygonReplay = replayGroup.getReplay(style.getZIndex(), 'Polygon');
        var setFillStrokeStyleSpy = sinon.spy(
          polygonReplay,
