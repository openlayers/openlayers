...
describe('ol.VectorRenderTile', function () {
  it('triggers "change" when previously failed source tiles are loaded', () =>
    new Promise((resolve) => {
      let sourceTile;
  ...
  it('sets ERROR state when source tiles fail to load', () =>
    new Promise((resolve) => {
      const source = new VectorTileSource({
        format: new GeoJSON(),
        url: 'spec/ol/data/unavailable.json',
  ...
  it("only loads tiles within the source tileGrid's extent", () =>
    new Promise((resolve) => {
      let tile;
