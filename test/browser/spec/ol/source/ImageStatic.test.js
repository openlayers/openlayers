...
describe('ol/source/ImageStatic', function () {
  ...
  describe('#getImage', function () {
    ...
    it('sets the scalable flag for a same-origin svg', (done) => {
      const source = new Static({
        url: '/spec/ol/source/images/cross.svg',
        imageExtent: [-29, -29, 29, 29],
      });
      source.on('imageloadend', () => {
        try {
          expect(source.scaleable).to.be(true);
          done();
        } catch (err) {
          done(err);
        }
    ...
    it('sets the scalable flag for a cors svg', (done) => {
      ...
      source.on('imageloadend', () => {
        try {
          expect(source.scaleable).to.be(true);
          expect(image.getImage().width).to.be(204);
          expect(image.getImage().height).to.be(377);
          expect(image.getImage()).to.be.a(HTMLImageElement);
          done();
        } catch (err) {
          done(err);
        }
    ...
    it('sets the scalable flag for a same-origin svg', (done) => {
      ...
      source.on('imageloadend', () => {
        try {
          expect(source.scaleable).to.be(true);
          expect(image.getImage().width).to.be(20);
          expect(image.getImage().height).to.be(20);
          expect(image.getImage()).to.be.a(HTMLImageElement);
          done();
        } catch (err) {
          done(err);
    ...
    it('sets the scalable flag for a data uri svg', (done) => {
      ...
      source.on('imageloadend', () => {
        try {
          expect(source.scaleable).to.be(true);
          expect(image.getImage().width).to.be(100);
          expect(image.getImage().height).to.be(100);
          expect(image.getImage()).to.be.a(HTMLImageElement);
          done();
        } catch (err) {
          done(err);
