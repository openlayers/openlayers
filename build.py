...
EXAMPLERULE = r'\A' + os.path.join('examples', '(?P<id>.*).json').encode('string-escape') + '\Z'
EXAMPLERULECOMBINE = r'\A' + os.path.join('examples', '(?P<id>.*).combined.js').encode('string-escape') + '\Z'

EXPORTS = [path
           for path in ifind('src')
           if path.endswith('.exports')
           if path != os.path.join('src','objectliterals.exports')]
...
@target('build/src/external/externs/types.js', 'bin/generate-exports', 'src/objectliterals.exports')
def build_src_external_externs_types_js(t):
    t.output('%(PYTHON)s', 'bin/generate-exports.py', '--externs', 'src/objectliterals.exports')


@target('build/src/external/src/exports.js', 'bin/generate-exports', 'src/objectliterals.exports', EXPORTS)
...
@target('build/src/internal/src/types.js', 'bin/generate-exports', 'src/objectliterals.exports')
...
@target('examples/example-list.js', 'bin/exampleparser', EXAMPLES)
...
def examples_star_json(name, match):
    def action(t):
        content = json.dumps({
            ...
            'inherits': '../base.json',
            'inputs': [
                os.path.join('examples', '%(id)s.js') % match.groupdict(),
                'build/src/internal/src/types.js',
            ],
        })
...
@rule(EXAMPLERULECOMBINE)
def examples_star_combined_js(name, match):
    def action(t):
        t.output('%(JAVA)s', '-jar', PLOVR_JAR, 'build', os.path.join('examples', '%(id)s.json') % match.groupdict())
        report_sizes(t)
    dependencies = [PLOVR_JAR, SRC, INTERNAL_SRC, 'base.json', os.path.join('examples', '%(id)s.js') % match.groupdict(), os.path.join('examples', '%(id)s.json') % match.groupdict()]
    return Target(name, action=action, dependencies=dependencies)
